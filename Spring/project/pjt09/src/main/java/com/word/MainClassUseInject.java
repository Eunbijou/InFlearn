...
public class MainClassUseInject {
	...
	public static void main(String[] args) {
		...
		GenericXmlApplicationContext ctx = new GenericXmlApplicationContext("classpath:appCtxUseInject.xml");

		WordRegisterServiceUseInject registerService = ctx.getBean("registerService",
				WordRegisterServiceUseInject.class);
		for (int i = 0; i < values.length; i++) {
			WordSet wordSet = new WordSet(keyWords[i], values[i]);
			registerService.register(wordSet);
		}

		WordSearchServiceUseInject searchService = ctx.getBean("searchService", WordSearchServiceUseInject.class);
